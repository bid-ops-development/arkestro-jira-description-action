...
import type { IGithubData, JIRADetails, PullRequestParams } from './types'
...
export class GithubConnector {
  ...
  getIssueKeyFromTitle(): { key: string, source: ESource } {
    ...
    let keyByPRTitle: string | null = null
    ...
    switch (WHAT_TO_USE) {
      ...
      case ESource.prTitle:
        ...
        break
      case ESource.both:
        keyByPRTitle = this.getIssueKeyFromString(prTitle)
        if (keyByPRTitle) {
  ...
  async updatePrDetails(details: JIRADetails): Promise<RestEndpointMethodTypes['pulls']['update']['response']> {
    ...
    return this.octokit.rest.pulls.update(prData)
  }
